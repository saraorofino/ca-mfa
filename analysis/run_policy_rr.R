...
run_policy <- function(params) {
  ...
  target_rr   <-0.25 #params$policy_rate
  implement_year_rr <-2025 #params$implement_year
  target_year_rr    <-2032 #params$target_year
  baseline_year_rr  <- 2023#params$baseline_year
  target_sector_rr <- 'pack' #params$target_sector
  ...
  calc_avoid_virgin(rc_perc_rr, is_scrap_consump = 0.5) # hard code in assumption for all states
  ...
  wasteman_rr <- calc_eol(wastegen_rr, recyc_output_rr, incineration)
