...
#' @param bau_rr_sect Recycling rate for the packaging sector per year based on CalRecycle data.
...
#' @description
#' Calculates the weight of recycling collected in each sector, currently only packaging recycled, per year. Uses the waste generation per year dataframe (wastegen) and the static recycling collection rate under business as usual dataframe. (bau_rr_sect). For business-as-usual pre-preprocessing leave out implement_year_rr, targer_rr and target_year_rr and ensure wastegen fed into the function is for BAU waste generation.
  ...
  collect_recyc <- wastegen_target |>
    mutate(
      rr_multiplier = case_when(
        target_rr <= baseline_rate ~ bau_rr_sect,
        year <= baseline_year_rr ~ bau_rr_sect,
    ...
    mutate(
      mt_plastic_collect = case_when(
        ...
        TRUE ~ mt_plastic_wastegen * bau_rr_sect
      )
    )
