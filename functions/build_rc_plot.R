...
build_rc_comparison_plot <- function(consum_bau, avoid_prod_rc, scenario_color) {
  ...
  rc_data <- tibble(
    Scenario = c("BAU", "Recycled Content"),
    mt_plastic = c(bau_total, bau_total - avoid_prod_rc)
  ) |>
    mutate(
      Scenario = factor(Scenario, levels = c("Recycled Content", "BAU")),
      y_num = as.numeric(Scenario) * 0.3 #adding a line spacing for visual
    )
  
  fill_values <- setNames(c(scenario_color, "black"), c("Recycled Content", "BAU")) #bau remains same across charts, change here if needed
